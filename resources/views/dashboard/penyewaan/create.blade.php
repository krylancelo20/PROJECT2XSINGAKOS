...
@section('container')
    ...
    <div class="row">
        <div class="col-lg-4 mb-5">
            <div class="card">
                @if ($kamar->image)
                    <img src="/foto/{{$kamar->image}}" alt="Foto Kamar {{ $kamar->tipe }}"
        ...
        <div class="col-lg-4">
            <form action="/dashboard/penyewaan" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                {{-- <div class="mb-3">
                    ...
                    <input type="month" class="form-control @error('awal_sewa') is-invalid @enderror" id="awal_sewa"
                        name="awal_sewa" min="{{ $awal }}" value="{{ $awal }}" onchange="awal()" required>
                    @error('awal_sewa')
                ...
                <div class="mb-3">
                    ...
                    <input type="month" class="form-control @error('akhir_sewa') is-invalid @enderror" id="akhir_sewa"
                        name="akhir_sewa" min="{{ $awal }}" value="" onchange="total()" required>
                    @error('akhir_sewa')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    <label for="awal_sewa" class="form-label">Dari :</label>
                    <input type="date" class="form-control @error('awal_sewa') is-invalid @enderror" id="awal_sewa"
                        name="awal_sewa" min="{{ $awal }}" value="{{ $awal }}" onchange="awal()" required>
                    @error('awal_sewa')
                        <div class="invalid-feedback">
                            {{ $message }}
                ...
                <div class="mb-3">
                    <label for="akhir_sewa" class="form-label">Sampai :</label>
                    <input type="date" class="form-control @error('akhir_sewa') is-invalid @enderror" id="akhir_sewa"
                        name="akhir_sewa" min="{{ $awal }}" value="" onchange="total()" required>
                    @error('akhir_sewa')
                        <div class="invalid-feedback">
                            {{ $message }}
    ...
    <script>
        ...
        const durasi_sewa = document.getElementById('text_durasi_sewa');
        ...
        function total() {
            let awal = new Date(awal_sewa.value);
            let akhir = new Date(akhir_sewa.value);
            let jadi = akhir.getMonth() - awal.getMonth() + 12 * (akhir.getFullYear() - awal.getFullYear())
            if (akhir.getDate() >= awal.getDate()) {
                ++jadi;
            }
            durasi_sewa.innerHTML = `${jadi} Bulan`;
        }
